...
def sanitize_text(text):
    """
    Remove ou traduz conteúdos com alfabetos proibidos.
    Se encontrar scripts proibidos, tenta traduzir para inglês.
    Se conseguir, retorna: [en: termo_traduzido]
    Se não conseguir, retorna Unknown.
    """
    if not text:
        return "Unknown"
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            translated_text = forbidden_pattern.sub("", translated_text)
            translated_text = re.sub(r'\s+', ' ', translated_text).strip()

            if translated_text:
                return f"[en: {translated_text}]"

    except Exception as e:
        logger.warning(f"Falha na tradução automática, aplicando Unknown: {e}")
...
def build_caption(user_name, title, album, artist):
    return (
...
async def inline_query(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    user_name = escape_html(sanitize_text(user.first_name if user else "Someone"))
    ...
    for i, track in enumerate(tracks[:10]):
        ...
        try:
            title_raw = sanitize_text(track["title"])
            artist_raw = sanitize_text(track["artist"]["name"])
            album_raw = sanitize_text(track["album"]["title"])
            cover = track["album"]["cover_big"]

            title = escape_html(title_raw)
            artist = escape_html(artist_raw)
            album = escape_html(album_raw)
...
async def select_track(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await cb_query.answer("Erro.", show_alert=True)
        return

    tracks = context.user_data.get("tracks")

    if not tracks or index >= len(tracks):
        await cb_query.answer("Resultado expirado. Busque novamente.", show_alert=True)
        return
    ...
    try:
        ...
        artist = escape_html(artist_raw)
        album = escape_html(album_raw)
        user_name = escape_html(sanitize_text(cb_query.from_user.first_name))
        ...
        logger.exception(f"Erro ao enviar foto/caption da música: {e}")
        await cb_query.answer("Erro ao enviar música.", show_alert=True)
