...
def connect_redis() -> None:
    ...
    try:
        redis_client = redis.Redis.from_url(
            REDIS_URL,
            decode_responses=True, # Mantido do bot base para estabilidade global
        )
        redis_client.ping()
...
def story_render_image(
    ...
    bot_name_clean = BOT_DISPLAY_NAME
...
async def story_fetch_cover(track: Dict[str, Any], query: Optional[str] = None) -> Image.Image:
    ...
    release_id = str(track.get("release_id") or "").strip()
    if release_id:
    ...
    if query and not has_high_res:
        ...
        if fallback_track:
            ...
            fallback_release_id = str(fallback_track.get("release_id") or "").strip()
...
async def show_search_results(update: Update, context: ContextTypes.DEFAULT_TYPE, query: str):
    ...
    for score, t in ranked[:5]:
        try:
            ...
            keyboard.append([
                InlineKeyboardButton(f"🎵 {title} — {artist}", callback_data=f"play:{track_id}")
            ])
        except Exception as e:
...
async def play(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if chat.type in ["group", "supergroup"]:
        cleanup_pending(now)
        PENDING_REPLIES[(chat.id, user.id)] = now
...
# =========================
# CLICKS GENÉRICOS (INCLUI FALLBACK LEGACY)
# =========================
...
async def click(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if len(data_parts) == 2:
            ...
            action, track_id = "play", cb.data
    except Exception:
        await cb.answer("⚠️ Ação inválida.", show_alert=True)
        return

    if action == "story":
        # Retrocompatibilidade de botões antigos: Redireciona graciosa para seleção de tema
        keyboard = [
            [
                InlineKeyboardButton("Modo Claro ⚪️", callback_data=f"story_theme:light:{track_id}"),
                InlineKeyboardButton("Modo Escuro ⚫️", callback_data=f"story_theme:dark:{track_id}")
    ...
    photo = (t.get("album") or {}).get("cover_big") or (t.get("album") or {}).get("cover_xl")
...
def _cover_candidates(track: Dict[str, Any]) -> List[str]:
    ...
    candidates = []
    for u in urls:
        if u and str(u).strip() not in seen:
            val = str(u).strip()
            seen.add(val)
...
async def top(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    lines = [f"📈 <b>Top global do {BOT_DISPLAY_NAME}</b>", ""]
    for i, ((track_id, score), meta) in enumerate(zip(entries, metas), 1):
        title = sanitize(meta.get("title") or f"Track {track_id}")
        artist = sanitize(meta.get("artist") or "Unknown")
        lines.append(f"{i}. 🎧 <b>{esc(title)}</b>")
...
async def post_init(application: Application):
    if not acquire_instance_lock():
        logger.warning("Aviso: Outra instância do bot parece ativa. Iniciando sem exclusividade garantida.")
...
def main():
    ...
    # Handlers Textuais de Grupo e Privado
    ...
    # Callbacks (Callbacks novos do Story e Fallback do Play)
    ...
    app.add_handler(CallbackQueryHandler(click)) # Fallback legados (ex: botão story: antigo redicerionado graciosamente)
